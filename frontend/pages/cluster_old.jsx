...
async function fetchK8sVersion() {
    try {
        const data = await callK8sApi('/version');
...
export default function Cluster() {
    ...
    useEffect(() => {
        const fetchNodeData = async () => {
            try {
                ...
                setNodeDetails(detailsResponse);

                // Calculate capacity AFTER both fetches have completed
                calculateCapacity(detailsResponse);
                calculateUsage(metricsResponse)
            } catch (error) {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const data = await callK8sApi('/api/v1/events');
    ...
    useEffect(() => {
        const fetchPods = async () => {
            try {
                const data = await callK8sApi('/api/v1/pods');
                setPods(data.items);
            } catch (error) {
