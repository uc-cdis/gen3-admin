...
func (c *tunnelServiceClient) Connect(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentMessage, ServerMessage], error) {
...
type TunnelService_ConnectClient = grpc.BidiStreamingClient[AgentMessage, ServerMessage]
...
func (UnimplementedTunnelServiceServer) Connect(grpc.BidiStreamingServer[AgentMessage, ServerMessage]) error {
...
func _TunnelService_Connect_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TunnelServiceServer).Connect(&grpc.GenericServerStream[AgentMessage, ServerMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TunnelService_ConnectServer = grpc.BidiStreamingServer[AgentMessage, ServerMessage]
